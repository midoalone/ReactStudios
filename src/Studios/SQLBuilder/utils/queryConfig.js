...
export default {
        ...
        AND: {
            ...
            formatConj: (children, conj, isForDisplay) => {
                return children.size > 1 ?
                    '(' + children.join(' '+ (isForDisplay ? "AND" : "AND") +' ') + ')'
                    : children.first();
            },
        },
        OR: {
            ...
            formatConj: (children, conj, isForDisplay) => {
                return children.size > 1 ?
                    '(' + children.join(' '+ (isForDisplay ? "OR" : "OR") +' ') + ')'
                    : children.first();
            },
        },
    },
        ...
        text: {
            widgets: {
                text: {
                    ...
                    widgetProps: {
                        formatValue: (val) => ("_"+JSON.stringify(val)),
                        valueLabel: "Text",
    ...
    operators: {
        ...
        select_any_in: {
            label: 'Any in',
            labelForFormat: 'IN',
            formatOp: (field, op, values, valueSrc) => {
                if (valueSrc == 'value')
        ...
        proximity: {
            ...
            formatOp: (field, op, values, valueSrc, valueType, opDef, operatorOptions) => {
